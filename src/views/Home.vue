...
export default {
  ...
  mounted () {
    this.$http
      .get(`http://http://157.230.107.7/api/runic_game_action/${this.hash_url_data}`)
    ...
    cardPlacing (card_position) {
      if (this.hand_card_number) {
        ...
        if (active_player == this.player_number) {
          ...
          setTimeout(() => {
            this.$http.post(`http://http://157.230.107.7/api/runic_game_action/${this.hash_url_data}`, {
          ...
          setTimeout(() => {
          this.$http.get(`http://http://157.230.107.7/api/runic_game_action/${this.hash_url_data}`)
          .then(response => {this.get_data(response)})
          }, 2000)
        }
